...
- `data_converter/`：AMASS/SMPL-H 到 `realtime_pose_v1` 源数据的转换脚本。
- `data_loaders/`：`realtime_pose_v1` task 生成、Dataset、normalizer、tracker pattern 规则。
...
- `eval/`：`realtime_pose_v1` 评估入口和指标计算。
...
- 张量维度超过两个语义轴时，在函数边界或关键变换附近写清形状，例如 `[B, C, T]`、`[T, 206]`、`[B, T, D]`。
...
## realtime_pose_v1 任务契约

- 固定为 60 帧历史条件 + 第 61 帧单帧补全。
- `seq_len = 61`，`target_start = 60`，`target_length = 1`。
- `feature_dim = 206`，模型输入输出均为 `[B, 206, 61]`。
- 通道 `0:144` 是 `body_pose_parent_6d`，`144:146` 是 `root_yaw_delta_sincos`。
- 通道 `146:164` 是 `tracker_pos_ref`，`164:200` 是 `tracker_rot_ref_6d`，`200:206` 是 `sensor_valid`。
- `inpaint_mask` 只允许覆盖第 61 帧的 `0:146`。
...
- 不保存 `contact`；训练和后处理需要时由 `joints_world` 动态派生。
- 不兼容旧 X277/current277 数据、task、checkpoint、Unity schema。
