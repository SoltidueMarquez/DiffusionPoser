...
当前主链路是 `realtime_pose_body_fbx_local_v1`，姿态表示为 `body_fbx_local_delta_6d`。旧 `realtime_pose_v2_contact` / `root_yaw_global_6d` source、task、normalizer、checkpoint、Unity runtime asset 不能和当前链路混用。
...
| `146:148` | 2 | `root_delta_xz_ref`，上一帧 root heading 参考系下 actor root XZ 位移 |
| `148:149` | 1 | `pelvis_height`，pelvis 世界高度 |
...
conda run -n diffusionposer5070 python -m data_converter.amass_to_realtime_pose `
  ...
  --output_dir dataset/AMASS_realtime_pose_body_fbx_local_60hz `
  --target_fps 60 `
  --overwrite

conda run -n diffusionposer5070 python -m data_loaders.generate_realtime_pose_tasks `
  --schema realtime_pose_body_fbx_local_v1 `
  --source_dir dataset/AMASS_realtime_pose_body_fbx_local_60hz `
  --output_dir dataset/AMASS_realtime_pose_body_fbx_local_60hz_tasks `
...
conda run -n diffusionposer5070 python -m data_loaders.compute_realtime_pose_normalizer `
  --schema realtime_pose_body_fbx_local_v1 `
  --task_dir dataset/AMASS_realtime_pose_body_fbx_local_60hz_tasks `
  --output_dir dataset/meta_AMASS_realtime_pose_body_fbx_local_60hz `
  --split train `
  --overwrite
```
...
conda run -n diffusionposer5070 python -m export.write_unity_runtime_assets `
  ...
  --normalizer_dir dataset/meta_AMASS_realtime_pose_body_fbx_local_60hz\<normalizer_run> `
  --normalize_input

conda run -n diffusionposer5070 python -m export.export_sentis_denoiser `
  --schema realtime_pose_body_fbx_local_v1 `
  --model_arch target_dit `
  --model_path runs\realtime_pose_body_fbx_local_target_dit\<run>\model000000001.pt `
  --output_dir ..\SIGGRAPH2024Unity\Assets\Projects\RealtimePose\Models\DiffusionPoser `
  --normalizer_dir dataset/meta_AMASS_realtime_pose_body_fbx_local_60hz\<normalizer_run>
```

Unity runtime 现在直接解码 body.fbx local delta：`bone.localRotation = restLocalRotation * decodedDelta`。`WorldOffset` 只用于显示偏移，不参与模型语义。
