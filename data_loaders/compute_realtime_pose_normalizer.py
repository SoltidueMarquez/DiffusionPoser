...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compute realtime_pose_v1 mean/std normalizer from converted source data.")
...
def compute_realtime_pose_normalizer(args: argparse.Namespace) -> dict[str, int | float | str]:
    ...
    if not source_dir.exists():
        raise FileNotFoundError(f"realtime_pose_v1 源数据目录不存在：{source_dir}")
    ...
    if not split_entries:
        raise RuntimeError(f"split={args.split} 没有匹配到 realtime_pose_v1 源文件。")
    ...
    for entry in tqdm(split_entries, desc=f"统计 split={args.split} realtime normalizer", unit="file"):
        source = load_realtime_source(Path(entry["source_path"]))
    ...
    normalizer = RealtimePoseNormalizer(base_dir=output_dir, eps=float(args.eps), disable=True)
