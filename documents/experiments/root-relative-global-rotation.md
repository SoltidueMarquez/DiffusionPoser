...
- 该实验不直接追求兼容旧 `realtime_pose_v2_contact` 数据、normalizer、checkpoint 或 Unity schema。

## 产物约定

- 不提交 `dataset/`、`runs/`、`output/`、`save/` 或 checkpoint/data 二进制产物。
- 训练、采样和评估命令默认使用 `conda run -n diffusionposer5070 <command>`。
- 新表示应使用独立 schema 名，避免覆盖 `realtime_pose_v2_contact` 的通道语义。

## 实验条目

### 001 - root-relative global rotation schema
