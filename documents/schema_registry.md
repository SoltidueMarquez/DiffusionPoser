...
新增 schema 时在 `schemas/<schema>/` 下建立并维护这些文件：
...
然后在 `schemas/registry.py` 注册 adapter。每个 trainable/exportable schema 必须同时具备：
...
当前 `realtime_pose_stationary5_v1` 的最小链路测试在 `tests/smoke/schemas/test_stationary5_train_export.py` 中覆盖 canonical exact name 和 legacy exact name。
...
AMASS 原始路径和生成产物路径通过 `configs/data_roots.local.json` 管理；没有 local 文件时读取 `configs/data_roots.example.json`。相对路径按仓库根目录解析：
...
`amass_root`、`smpl_model_dir` 和 `body_fbx_rest_json` 指向原始输入或 Unity 导出的 rest 数据，不放进 `generated_root`。`generated_root` 下只放可重建产物：

| 类型 | 路径模板 |
| --- | --- |
| source | `generated_root/sources/<schema_name>/<source_set_name>` |
| task | `generated_root/tasks/<schema_name>/<task_set_name>` |
| normalizer | `generated_root/normalizers/<schema_name>/<normalizer_name>` |
| training run | `runs/<schema_name>/<experiment_name>` |
...
- 只有 registry 中明确注册的 legacy schema 才允许继续维护。当前 legacy exact name 是 `realtime_pose_body_fbx_local_root_y0_v1`，canonical name 是 `realtime_pose_stationary5_v1`。
...
- legacy schema 要继续 trainable/exportable，就必须保留 README、adapter、contract smoke、最小训练测试和最小导出测试。
