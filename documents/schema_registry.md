...
AMASS 原始路径、runtime contract 和生成产物路径统一通过 `configs/artifact_roots.local.json` 管理；没有 local 文件时读取 `configs/artifact_roots.example.json`。相对路径按仓库根目录解析：
...
`amass_root`、`smpl_model_dir` 和 `body_fbx_rest_json` 指向受管理的原始输入或 runtime contract；`body_fbx_rest_json` 必须是 Unity 导出文件在 artifact store 中经过校验的镜像。`generated_root` 下只放可重建产物。若复制下面命令，默认省略 `--artifact_roots_config`，让脚本自动选择 local 或 example；需要固定本机路径时，先创建 `configs/artifact_roots.local.json`，再显式添加该参数。
...
| training run root | `runs_root/<schema_name>/<experiment_name>` |
| export | `outputs_root/<schema_name>/<export_name>` |
...
`scripts.run_realtime_pose_pipeline` 还使用 `--experiment_name` 和 `--export_name` 推导 `runs_root/<schema>/<experiment>` 与 `outputs_root/<schema>/<export>`。显式传入 `--source_dir`、`--task_dir`、`--normalizer_dir`、`--save_dir` 或 `--export_dir` 会覆盖推导路径，但不会放宽 metadata 校验。直接调用导出脚本时，`--normalizer_dir` 应指向具体 normalizer 产物目录；pipeline 会在传参前把 normalizer set 根解析到 latest 产物目录。
