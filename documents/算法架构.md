...
离线训练任务允许物化 (K\in[1,15]) 个连续目标，默认 (K=4)。K15 不改变模型窗口：每一步仍读取60帧历史并预测一个144D姿态，只把 `deployed_pred_xstart.detach()` 写回历史后重复执行。默认 `rollout_frame_weighting=uniform` 时训练目标保持“第1帧损失 + 第2～K帧平均损失 + 现有速度/旋转速度/foot-slide项”；受控实验可把后续帧主 loss 切换为 `linear_late`，令第 (s) 个后续预测的权重为 (s/\sum_{j=1}^{K-1}j)，总权重仍为1。该策略不改变各项时序 loss，也不会因 K 增大而把后续帧损失简单求和放大。冷启动第 (s) 个预测的有效历史长度为 `min(60,H+s)`；整段 `configured/measured_valid/d_off/d_on/hard_rotation_state` 必须从同一75帧虚拟会话连续重放。

15帧 exposure-bias 实验采用严格配对：R4-Control 与 R15 读取同一 K15 task、同一逐字节复用 normalizer、同一正式 B0 和相同 batch/seed/loss，仅 `rollout_steps=4/15` 不同。它们都从 B0 独立训练，不能相互续训或从旧 B1 pilot 初始化。
...
- Head trajectory；
- 每个 Tracker 的当前 state/position/rotation token；
- DynamicObservationEncoder 已编码的逐 Tracker 60帧 `history_summary:[B,6,D]`；
- Anchor region coverage。

每个 Tracker 先把 `[state,position,rotation,history_summary]` 从 `4D` 融合回
`D`，再乘连续 `alpha` 并除以 `sum(alpha)`。六个结果按 Head、LeftHand、
RightHand、Hip、LeftFoot、RightFoot 的固定顺序保留，展平为 `[B,6D]` 后由
无 bias 的 `anchor_slot_projection:[D,6D]` 压回 `[B,D]`。投影初始值是六个
单位矩阵横向拼接，所以训练开始时严格复现旧加权平均；训练后才允许左右手、
双脚等槽位形成不同作用。因此 Uncertain、Missing 和未配置 Tracker 的当前与
历史测量都不能通过共享聚合泄漏进 Prior。Observation Encoder 继续冻结，B1只
训练Prior如何消费其既有B0历史特征。轻量 Prior 输出：
...
左右脚 contact logits，`v` 为训练期24关节速度辅助头。`r.xyz` 由 Root MLP
预测，`r.yaw` 则直接从 `X` 的 Pelvis rotation 解码，因而 FK、innovation 和
TargetDiT adapter 只消费同一个 Root yaw 真值。由 `X/r` 和目标骨架 offsets
执行内部可微 FK，得到各 Tracker 的预测位置与旋转；其中 `r.xyz` 是 TAID 内部
辅助状态，不覆盖 runtime Root。

B1 只训练 Prior，监督包括 coverage 加权 SO(3) pose、Root xyz/yaw、joint
velocity 和 contact BCE。唯一进入总损失的 FK joint L1 使用 hard rotation
projection 后的 144D pose，并调用与 runtime 相同的 Head-Anchored Resolver；
内部 `X/r` FK 仅报告 innovation 质量和两套 Root/关节路径的差距，不参与加权。
B2～B6冻结Prior；所有进入FK residual和TargetDiT条件的Prior输出都使用
stop-gradient。

当前跨 Tracker 聚合使用固定六槽投影，运行参数固定记录为
`taid_prior_tracker_aggregation=fixed_slots`。29Z及更早 B1 checkpoint 不含新增
投影参数，不能恢复训练；正式 B0 不包含 TAID 参数，仍可初始化当前 B1。本轮没有
同时修改 velocity监督、K15 rollout、loss权重或后续 TargetDiT 条件路径。

当前实现明确选择“部署 Resolver 为 Root 真值”：Head 世界位置严格对齐、Actor
Root 位于 floor，部署 Root 不读取 `r.xyz`。这与 proposal 中让 `r.xyz` 辅助
Root Resolver 的建议存在受控差异，但保持了144D、Projected DDIM和Unity稳定
接口；若以后要改变该选择，必须作为新的稳定接口契约单独审计。
