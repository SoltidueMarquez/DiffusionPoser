...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate realtime_pose_v1 reconstruction result npz files.")
...
def evaluate_file(path: Path) -> dict[str, float | int | str]:
    with np.load(path, allow_pickle=False) as data:
        reference, reconstructed, feature_space = read_feature_pair(data, path)
    ...
    target_mask = inpaint_mask[:, :, :REALTIME_POSE_TARGET_DIM].any(axis=2)
    ...
    target_mae = float(np.mean(np.abs(reconstructed[target_mask, :REALTIME_POSE_TARGET_DIM] - reference[target_mask, :REALTIME_POSE_TARGET_DIM])))
    return {
        "path": str(path),
...
def read_feature_pair(data, path: Path) -> tuple[np.ndarray, np.ndarray, str]:
    """
    优先读取 raw 成对字段。
    ...
    for feature_space, reference_keys, reconstructed_keys in FEATURE_ARRAY_GROUPS:
        if not set(reference_keys).isdisjoint(files) and not set(reconstructed_keys).isdisjoint(files):
            reference = read_feature_array(data, path, reference_keys)
            reconstructed = read_feature_array(data, path, reconstructed_keys)
            return reference, reconstructed, feature_space
    raise KeyError(f"{path} 缺少可配对的 reference/reconstructed 特征字段。")


def read_feature_array(data, path: Path, keys: tuple[str, ...]) -> np.ndarray:
    for key in keys:
    ...
    if array.ndim == 2 and array.shape[1] == REALTIME_POSE_INPUT_DIM:
        return array[None]
    if array.ndim == 3 and array.shape[2] == REALTIME_POSE_INPUT_DIM:
        return array
    if array.ndim == 3 and array.shape[1] == REALTIME_POSE_INPUT_DIM:
        return np.transpose(array, (0, 2, 1))
    raise ValueError(f"{path} 特征应为 [T,{REALTIME_POSE_INPUT_DIM}] 或 [B,T,{REALTIME_POSE_INPUT_DIM}]，实际为 {array.shape}")
