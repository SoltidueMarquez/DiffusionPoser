param(
    ...
    [int]$LogInterval = 100,
    [string]$WindowTitle = "DiffusionPoser Stage B geometry loss probe"
)
...
$resumeCheckpoint = Join-Path $RunDir "model000025000.pt"
$logPath = Join-Path $RunDir "stage_b_geometry_loss.stdout.log"
...
$trainArgs = @(
    ...
    "--save_dir", $RunDir,
    "--run_name", "stage_b_geometry_loss_probe_seed10",
    "--batch_size", "16",
    "--num_workers", "0",
    ...
    "--pelvis_head_xz_loss_weight", "2.0",
    "--pelvis_head_height_loss_weight", "1.0",
    "--lower_body_fk_loss_weight", "1.0",
    "--lower_body_rot_loss_weight", "0.5",
    "--geometry_huber_beta", "0.05",
    "--foot_contact_velocity_loss_weight", "0.5",
    "--foot_contact_height_loss_weight", "0.25",
    "--foot_contact_height_threshold", "0.05",
    "--foot_contact_velocity_huber_beta", "0.05",
    "--foot_contact_height_huber_beta", "0.01",
    "--eval_num_batches", "4",
    "--cuda", "true",
    "--device", "0",
...
Write-Host "Stage B geometry-loss command:"
Write-Host "$python $($trainArgs -join ' ')"
Write-Host "Log: $logPath"
Write-Host "Run directory: $RunDir"

# Windows PowerShell 5.1 turns native stderr warnings into ErrorRecord objects.
# Keep them visible without treating harmless PyTorch warnings as fatal.
