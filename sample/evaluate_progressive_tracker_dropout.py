...
PROGRESSIVE_DROP_ORDERS = tuple(
    tuple(order) for order in permutations(OPTIONAL_TRACKER_NAME_TO_INDEX)
)
PROGRESSIVE_DROP_ORDER_NAMES = tuple(
    "_then_".join(order) for order in PROGRESSIVE_DROP_ORDERS
)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "评估同一长序列中 Tracker 从六点逐阶段退化到三点的闭环表现。"
        )
    )
    ...
    group = parser.add_argument_group("progressive tracker dropout")
    ...
    group.add_argument(
        "--drop_orders",
        nargs="+",
        choices=("all", *PROGRESSIVE_DROP_ORDER_NAMES),
        default=["all"],
        help="选择 optional Tracker 移除顺序；默认运行全部六种排列。",
    )
    group.add_argument("--output_json", required=True)
    return parser


def resolve_drop_order_names(requested: list[str]) -> tuple[str, ...]:
    values = tuple(str(value) for value in requested)
    if "all" in values:
        if values != ("all",):
            raise ValueError("--drop_orders all 不能与具体移除顺序同时使用。")
        return PROGRESSIVE_DROP_ORDER_NAMES
    if not values:
        raise ValueError("至少需要一个 Tracker 移除顺序。")
    if len(set(values)) != len(values):
        raise ValueError("--drop_orders 不能包含重复顺序。")
    return values


def build_progressive_dropout_protocols(
    drop_order_names: tuple[str, ...] | list[str] | None = None,
) -> tuple[TrackerEvaluationProtocol, ...]:
    selected = (
        PROGRESSIVE_DROP_ORDER_NAMES
        if drop_order_names is None
        else tuple(drop_order_names)
    )
    order_lookup = dict(zip(PROGRESSIVE_DROP_ORDER_NAMES, PROGRESSIVE_DROP_ORDERS))
    protocols: list[TrackerEvaluationProtocol] = []
    for order_name in selected:
        if order_name not in order_lookup:
            raise ValueError(f"未知 Tracker 移除顺序：{order_name}")
        drop_order = order_lookup[order_name]
        drop_indices = tuple(
            OPTIONAL_TRACKER_NAME_TO_INDEX[name] for name in drop_order
        )
        stages = build_progressive_stage_definitions(drop_order)

        def build_schedule(
            scored_frame_count: int,
            *,
            current_drop_indices: tuple[int, ...] = drop_indices,
        ) -> TrackerSequenceSchedule:
            return build_equal_quarter_tracker_schedule(
                scored_frame_count=scored_frame_count,
                drop_order=current_drop_indices,
            )

        protocols.append(
            TrackerEvaluationProtocol(
                name=f"drop_{order_name}",
                warmup_tracker_available=tuple(ALL_SIX_AVAILABLE),
                stages=stages,
...
def build_progressive_stage_definitions(
    ...
    if len(drop_order) != len(OPTIONAL_TRACKER_INDICES) or set(drop_order) != set(
        OPTIONAL_TRACKER_NAME_TO_INDEX
    ):
        raise ValueError("drop_order 必须恰好包含 Hip、Left Foot 和 Right Foot。")
    available = np.asarray(ALL_SIX_AVAILABLE, dtype=bool).copy()
    stages: list[TrackerStageDefinition] = []
    for stage_index in range(PROGRESSIVE_STAGE_COUNT):
        if stage_index > 0:
            dropped_name = drop_order[stage_index - 1]
            available[OPTIONAL_TRACKER_NAME_TO_INDEX[dropped_name]] = False
        stages.append(
            TrackerStageDefinition(
                index=stage_index,
                name=f"trackers_{TRACKER_COUNT - stage_index}",
                tracker_available=tuple(bool(value) for value in available.tolist()),
                metadata={"dropped_trackers": list(drop_order[:stage_index])},
            )
        )
    return tuple(stages)
...
def build_equal_quarter_tracker_schedule(
    *,
    scored_frame_count: int,
    drop_order: tuple[int, ...],
) -> TrackerSequenceSchedule:
    """把正式计分帧等分四段，依次移除一个 optional Tracker。"""
    ...
    normalized_order = tuple(int(index) for index in drop_order)
    if len(normalized_order) != len(OPTIONAL_TRACKER_INDICES) or set(
        normalized_order
    ) != set(OPTIONAL_TRACKER_INDICES):
        raise ValueError("drop_order 必须是三个 optional Tracker index 的排列。")
    ...
    for stage_index in range(PROGRESSIVE_STAGE_COUNT):
        if stage_index > 0:
            available[normalized_order[stage_index - 1]] = False
...
def format_progressive_order_report(report: dict) -> dict:
    """把共享内核结果整理成动态实验面向读者的 JSON 层次。"""

    return {
        "name": report["name"],
        "drop_order": list(report["drop_order"]),
...
def summarize_progressive_stages(reports: list[dict]) -> list[dict]:
    ...
    for stage_index in range(PROGRESSIVE_STAGE_COUNT):
        ...
        tracker_count = TRACKER_COUNT - stage_index
        if any(int(stage["tracker_count"]) != tracker_count for stage in stage_reports):
            raise RuntimeError("不同移除顺序的阶段 Tracker 数量不一致。")
        summaries.append(
            {
...
def ground_truth_jitter_reference(reports: list[dict]) -> float:
    ...
    if not np.isfinite(values).all() or not np.allclose(
        ...
        raise RuntimeError("六种移除顺序的 GT Jitter reference 必须一致。")
...
def _summarize_metric_dicts(metrics: list[dict]) -> dict:
    ...
    return {"mean": mean, "std_across_drop_orders": std}
...
def main(argv: list[str] | None = None) -> dict:
    ...
    selected_order_names = resolve_drop_order_names(list(args.drop_orders))
    fixseed(args.seed)
    device = torch.device(
    ...
    protocols = build_progressive_dropout_protocols(selected_order_names)
    ...
    payload = {
        "experiment": "progressive_tracker_dropout_6_to_3",
        ...
        "tracker_policy": (
            "all six during warmup; scored current-frame masks follow equal quarters"
        ),
        "sampling_noise_policy": "common_random_numbers",
        "sampling_noise_seed": int(args.seed),
        "schedule": {
            "stage_policy": "equal_scored_quarters",
            "warmup_tracker_available": list(ALL_SIX_AVAILABLE),
            "warmup_tracker_count": TRACKER_COUNT,
            "tracker_counts": [6, 5, 4, 3],
            "minimum_scored_frames": MIN_PROGRESSIVE_SCORED_FRAMES,
            "drop_orders": [
                list(protocol.metadata["drop_order"]) for protocol in protocols
            ],
        },
    ...
    print(f"[progressive-dropout] wrote {output}")
    return payload
